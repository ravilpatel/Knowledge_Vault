...
const PAGE_SIZE = 40;
const TABLES = ['notes','people','companies','technologies','projects','categories','tags','expenses','todos','panels','panel_fields','panel_entries'];
...
let state = {
  ...
  expenses:[], todos:[], userSettings:null,
...
async function sbFetchAll() {
  ...
  try {
    ...
    state.panels = (results[9].data||[]).sort((a,b)=>a.sort_order - b.sort_order);
    state.panel_fields = results[10].data||[];
    state.panel_entries = results[11].data||[];
...
function renderUserInfo() {
  ...
  document.getElementById('sbUserInfo').innerHTML = `
    <div class="su-avatar" style="width:32px; height:32px; border-radius:50%; background:var(--primary); color:var(--on-primary); display:flex; align-items:center; justify-content:center; font-weight:600; font-size:14px;" title="${email}">${initial}</div>
  `;
}
...
function renderMainHeader() {
  ...
  } else if(['all','finance','finance-report','matrix','todos','news'].includes(state.currentView)) {
    vc.textContent=state.currentView === 'all' ? state.panels.length + ' panels' : '';
    sc.classList.add('hidden'); af.innerHTML='';
    if (state.currentView === 'all') {
      va.innerHTML = `
        <button onclick="toggleMinimalMode()" class="btn-secondary" style="display:inline-flex; align-items:center; gap:6px; padding:6px 12px; font-size:13px; cursor:pointer;">
          <i class="fas ${state.minimalMode ? 'fa-expand' : 'fa-compress'}"></i>
          <span>${state.minimalMode ? 'Full Dashboard' : 'Minimal Mode'}</span>
        </button>
      `;
    } else {
      va.innerHTML = '';
...
function renderNavPanels() {
  ...
  const notesPanel = state.panels.find(p => p.name === 'Notes');
  const otherPanels = state.panels.filter(p => p.name !== 'Notes');
  ...
  if (notesPanel) {
    ...
    primaryContainer.innerHTML = `
      <a href="#" class="tn-link ${isActive ? 'active' : ''}" data-view="panel-${notesPanel.id}" onclick="switchView('panel-${notesPanel.id}'); return false;">
        Notes <span class="count">${entryCount}</span>
      </a>
    `;
  } else {
    primaryContainer.innerHTML = '';
...
function getGreeting() {
  ...
  const name = currentUser ? (currentUser.email.split('@')[0]) : 'User';
...
function renderPanelsColumns() {
  ...
  const pinnedCardsHtml = (state.pinnedPanelIds || []).map(pId => {
    ...
    if (panel.name === 'Notes') {
      ...
      subtext = `${activeNotes.length} notes`;
  ...
  state.notes.filter(n => n.status !== 'deleted').slice(0, 10).forEach(n => {
    activities.push({
      text: `You ${n.created_at === n.updated_at ? 'created note' : 'edited note'} "<strong>${escHtml(n.title || 'Untitled')}</strong>"`,
  ...
  state.expenses.slice(0, 10).forEach(e => {
    const currency = state.userSettings?.finance_currency || 'INR';
    const symbol = currency === 'INR' ? '₹' : (currency === 'EUR' ? '€' : '$');
    activities.push({
      text: `You added expense "<strong>${escHtml(e.description || 'Expense')}</strong>" (${symbol}${e.amount})`,
      time: new Date(e.date || e.created_at)
    });
  });
  ...
  if (!state.minimalMode) {
    ...
    const formattedExpense = currencySymbol + totalExpenses.toLocaleString('en-IN');
    ...
    dashboardHtml = `
      ...
      <div class="dash-stats-grid">
        <div class="stat-card">
          <div class="stat-info">
            ...
            <span class="stat-sub">Total Notes</span>
          </div>
        ...
        <div class="stat-card">
          <div class="stat-info">
            ...
            <span class="stat-val" style="font-size:19px;">${formattedExpense}</span>
            <span class="stat-sub">This Month</span>
          </div>
          <div class="stat-icon-wrapper" style="background:rgba(236,72,153,0.08); color:var(--accent-pink);">
      ...
      <div class="split-sections">
        <div class="recent-notes-box">
          <div class="dash-section-title" style="margin-bottom:14px;"><i class="fas fa-file-alt"></i> Recent Notes</div>
          ${recentNotesHtml.length === 0 ? `<div style="text-align:center; padding:16px; color:var(--ink-faint); font-size:13px; font-style:italic;">No notes found</div>` : recentNotesHtml}
          ${recentNotesHtml.length > 0 ? `<div style="margin-top:12px; text-align:left;"><a href="#" onclick="switchView('panel-${state.panels.find(p => p.name === 'Notes')?.id || ''}'); return false;" style="font-size:12px; font-weight:600; color:var(--primary); text-decoration:none;">View all notes &rarr;</a></div>` : ''}
        </div>
